...
const UpdateTour = () => {
    ...
    const handleValidSubmit = (e, values) => {
        ...
        formdata.append("startDates", !values.endDate ? filterTour[0]?.startDates[1] : values.endDate);

        formdata.append("name", !values.tourName ? filterTour[0]?.tourName : values.tourName);
        formdata.append("price", !values.price ? filterTour[0]?.price : values.price);
        formdata.append("duration", !values.duration ? filterTour[0]?.duration : values.duration);
        formdata.append("maxGroupSize", !values.groupSize ? filterTour[0]?.maxGroupSize : values.groupSize);
        formdata.append("difficulty", !values.type ? filterTour[0]?.difficulty : values.type);
        formdata.append("priceDiscount", !values.priceDiscount ? filterTour[0]?.priceDiscount : values.priceDiscount);
        formdata.append("summary", !values.summary ? filterTour[0]?.summary : values.summary);
        formdata.append("description", !values.description ? filterTour[0]?.description : values.description);
    ...
    return (
        <Layout>
            <Container fluid>
                <Link to="/admin-tour" className="mb-5 pb-5"> <BsArrowLeft size={22} /> Back </Link>
                <AvForm className="mt-5" onValidSubmit={(e, v) => { handleValidSubmit(e, v) }}>
                    <Card>
                        <CardBody>
